...
def int_to_bytes(number: int) -> bytes:
    return number.to_bytes(length=(16 + (number + (number < 0)).bit_length()) // 16, byteorder='big', signed=True)
...
class Coms_Manager():
    def __init__(self, Types):
        ...
        if Types:
            for Type in Types:
                if Type == "SerialLine":
                    self.coms.append(SerialLine("COM3", 9600))
                elif Type == "UDPSocket":
                    self.coms.append(UDPSocket(udpIP="127.0.0.1", portTX=8000, portRX=8001, enableRX=False, suppressWarnings=True))
...
class SerialLine():
    ...
    def Send_Bytes(self, values):
        if self.serial.is_open: # Only sends if serial is open
            vals = [translate(((-1)*values[0]),-100,100,65,130), translate(values[1],-100,100,0,180), translate(((-1)*values[2]),-15,-5,50,135)]
            send = str(vals[2])+"a" + str(vals[2])+"b" + str(vals[2])+"c" + str(vals[2])+"d" + str(vals[0])+"g" + str(vals[1])+"i" + "t"
            print(send)
...
class UDPSocket():
    ...
    def Send_Bytes(self,targets):
        temp = list(targets).copy()
        temp2 = targets[1]
        targets[1] = 0
        temp.append(temp2)
    ...
    def SendData(self, strToSend):
        ...
        # print(strToSend, " -- ", bytes(strToSend,'utf-8'))
