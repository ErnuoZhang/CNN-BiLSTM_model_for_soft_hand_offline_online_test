...
def calc_vec_angle(a, b, ref):
    angle = np.dot(a, b) / (scipy.linalg.norm(a)*scipy.linalg.norm(b))
    if angle < 0: angle = np.maximum(-1, angle) #Bounds the values to avoid erros due to numerical precision - Bruno
    if angle > 0: angle = np.minimum(1, angle) #Bounds the values to avoid erros due to numerical precision - Bruno
    angle =  np.arccos(angle)
    sign = np.sign(np.cross(a, b).dot(ref))
    # print(angle *(180/np.pi) ,np.cross(a, b), " -- ", np.cross(a, b).dot(ref))
    # print(angle *(180/np.pi) , sign)
    if sign != 0:
        return sign* angle
    else:
        return angle
