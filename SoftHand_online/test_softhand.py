...
softhand_serial = init_device("COM5")
# softhand_serial.open()

ID = 1

activate_device(softhand_serial,ID,True)

time.sleep(3)

set_motor_inputs(softhand_serial,ID,-5417,4367)

time.sleep(3)

set_motor_inputs(softhand_serial,ID,0,0)

time.sleep(3)

activate_device(softhand_serial,ID, False)
softhand_serial.close()
